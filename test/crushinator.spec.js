...
describe('crushinator', () => {
  describe('crushable', () => {
    imageHosts.forEach((imageHost) => {
      test(`should approve images on ${imageHost}`, () => {
        expect(crushinator.crushable(`http://${imageHost}/images/test.jpg`)).toBeTruthy();
        expect(crushinator.crushable(`https://${imageHost}/images/test.jpg`)).toBeTruthy();
      });
    });
  ...
  describe('uncrush', () => {
    ...
    test('should leave uncrushed images intact', () => {
      expect(crushinator.uncrush('http://test.com/images/test.jpg')).toEqual('http://test.com/images/test.jpg');
    });
  ...
  describe('crush', () => {
    ...
    test('should avoid double-crushing images', () => {
      ...
      expect(crushinator.crush(url, { width: 640 })).toEqual(`${crushed}?w=640`);
    });

    test('should update old Crushinator URLs to the new host', () => {
      const url = 'https://img-ssl.tedcdn.com/r/images.ted.com/image.jpg';
      expect(crushinator.crush(url, { width: 320 })).toEqual('https://pi.tedcdn.com/r/images.ted.com/image.jpg?w=320');
    });
    ...
    describe('Host testing', () => {
      imageHosts.forEach((imageHost) => {
        test(
          `should provide secure Crushinator URLs for images hosted on ${imageHost}`,
          () => {
    ...
    describe('with configurations', () => {
      test('should use the crushinator host override', () => {
        ...
        expect(crushinator.crush('https://images.ted.com/image.jpg')).toEqual('https://example.com/r/images.ted.com/image.jpg');
      });
    });
    ...
    describe('with defaults', () => {
      ...
      describe('enabled via config', () => {
        ...
        test('should use the default options', () => {
          expect(crushinator.crush(uncrushed)).toEqual(`${crushed}?${defaultParams}`);
        });
      });
      ...
      describe('enabled via options, disabled via config', () => {
        ...
        test('should use the default options', () => {
          expect(crushinator.crush(uncrushed, { defaults: true })).toEqual(`${crushed}?${defaultParams}`);
        });
      });
      ...
      describe('disabled via options, enabled via config', () => {
        ...
        test('should not use default options', () => {
          expect(crushinator.crush(uncrushed, { defaults: false })).toEqual(crushed);
        });
      });
      ...
      describe('enabled but partially overridden by options', () => {
        ...
        test('should use the default options', () => {
          expect(crushinator.crush(uncrushed, { quality: 99 })).toEqual(`${crushed}?u%5Br%5D=2&u%5Bs%5D=0.5&u%5Ba%5D=0.8&u%5Bt%5D=0.03&quality=99`);
        });
      });
    });
    ...
    describe('options API', () => {
      test('should recognize the width option', () => {
        expect(crushinator.crush(uncrushed, { width: 300 })).toEqual(`${crushed}?w=300`);
      });

      test('should recognize the height option', () => {
        expect(crushinator.crush(uncrushed, { height: 300 })).toEqual(`${crushed}?h=300`);
      });

      test('should recognize the quality option', () => {
        expect(crushinator.crush(uncrushed, { quality: 90 })).toEqual(`${crushed}?quality=90`);
      });

      test('should support multiple options', () => {
        expect(crushinator.crush(uncrushed, { width: 640, height: 480, quality: 90 })).toEqual(`${crushed}?w=640&h=480&quality=90`);
      });

      test('should recognize the crop size options', () => {
        expect(crushinator.crush(uncrushed, { crop: { width: 320, height: 240 } })).toEqual(`${crushed}?precrop=320%2C240`);
      });

      test('should recognize the crop location options', () => {
        expect(crushinator.crush(uncrushed, {
          crop: {
            width: 320,
            height: 240,
            x: 250,
      ...
      test('should recognize the align option', () => {
        expect(crushinator.crush(uncrushed, { align: 'top' })).toEqual(`${crushed}?gravity=n`);

        expect(crushinator.crush(uncrushed, { align: 'bottom' })).toEqual(`${crushed}?gravity=s`);

        expect(crushinator.crush(uncrushed, { align: 'left' })).toEqual(`${crushed}?gravity=w`);

        expect(crushinator.crush(uncrushed, { align: 'right' })).toEqual(`${crushed}?gravity=e`);

        expect(crushinator.crush(uncrushed, { align: 'middle' })).toEqual(`${crushed}?gravity=c`);
      });

      test(
        'should allow custom alignment to override the gravity selected by the fit option',
        () => {
          expect(crushinator.crush(uncrushed, {
            width: 320,
            height: 240,
      ...
      test('should recognize the unsharp option', () => {
        ...
        })).toEqual(`${crushed}?u%5Br%5D=1.4&u%5Bs%5D=0.7&u%5Ba%5D=0.8&u%5Bt%5D=0.5`);

        expect(crushinator.crush(uncrushed, {
          unsharp: true,
        })).toEqual(`${crushed}?u%5Br%5D=2&u%5Bs%5D=0.5&u%5Ba%5D=0.8&u%5Bt%5D=0.03`);
      });
    });
  });
});
